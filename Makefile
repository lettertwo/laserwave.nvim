...
NVIM_VERSION := 0.10.0
LUA_PATH := './lua_modules/share/lua/5.1/?.lua;./lua_modules/share/lua/5.1/?/init.lua;./lua/?.lua;./lua/?/init.lua;;'
...
./vendor/lush.nvim:
	@mkdir -p $@
	@git clone git@github.com:rktjmp/lush.nvim.git $@
...
.PHONY: build
build: ./lua_modules/bin/nlua ./vendor/lush.nvim ./vendor/shipwright.nvim
	@$(word 1,$^) -e 'vim.opt.rtp:prepend({vim.fn.getcwd(), "$(word 2,$^)", "$(word 3,$^)"})' -e 'require("shipwright").build()'
